...
import Chart from "../components/interview/chart";

export default function InterviewScore() {
  const { state: data } = useLocation();
  const categories = Object.keys(data);
  const total = categories.map(category => data[category].total);
  const correctCounts = categories.map(category => data[category].correct);
  ...
  return (
    <Wrapper>
      <ResultBox>
        ...
        <Hr />
        <ResultTitle>
          {total}문제중 {correctCounts}문제 맞췄어요 <Clap src={clap} alt="clap image" width={25} />
        </ResultTitle>
        <DescBox>
          ...
          <TabContext value={selected}>
            <Box>
              <TabList aria-label="lab API tabs example">
                <Tab label="문제의 정답" value="1" onClick={() => setSelected("1")} sx={{ fontSize: "20px", color: selected !== "1" ? "#7f7f7f" : "" }} />
                <Tab label="면접 결과 분석" value="2" onClick={() => setSelected("2")} sx={{ fontSize: "20px", color: selected !== "2" ? "#7f7f7f" : "" }} />
              </TabList>
            </Box>
            <TabPanel value="1">
